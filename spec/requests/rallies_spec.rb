...
describe RalliesController, type: :request do
  ...
  describe 'GET #index' do
    it 'indexアクションにリクエストすると正常にレスポンスが返ってくる' do
      get rallies_path, headers: @headers
  ...
  describe 'GET #show' do
    it 'showアクションにリクエストすると正常にレスポンスが返ってくる' do
      get rally_path(@rally), headers: @headers
      expect(response.status).to eq 200
    end
  ...
  describe 'GET #edit' do
    it 'editアクションにリクエストすると正常にレスポンスが返ってくる' do
      get edit_rally_path(@rally), headers: @headers
      expect(response.status).to eq 200
    end
  ...
  describe 'PUT #update' do
    it 'updateアクションにリクエストすると正常にレスポンスが返ってくる' do
      put rally_path(@rally), headers: @headers,
                              params: { rally: @rally.attributes }
      expect(response).to have_http_status(:found)
    end
    it 'updateアクションに不適切なパラメータを送るとレンダリングされる' do
      put rally_path(@rally), headers: @headers, params: { rally: { title: '' } }
      expect(response.status).to eq 422
      expect(response).to render_template(:edit)
    end
    it '「更新する」ボタンを押すとdraftがfalseになる' do
      put rally_path(@rally), headers: @headers,
                              params: { rally: @rally.attributes, publish: true }
      @rally.reload
      expect(@rally.draft).to be false
    end
    it '「下書きに戻す」ボタンを押すとdraftがtrueになる' do
      put rally_path(@rally), headers: @headers,
                              params: { rally: @rally.attributes, save_as_draft: true }
      @rally.reload
  ...
  describe 'GET #new' do
    it 'newアクションにリクエストすると正常にレスポンスが返ってくる' do
      get new_rally_path, headers: @headers
      expect(response.status).to eq 200
    end
  ...
  describe 'POST #create' do
    it 'createアクションにリクエストすると正常にレスポンスが返ってくる' do
      post rallies_path, headers: @headers,
                         params: { rally: { title: 'test', abstract: 'test', conclusion: 'test', opinion: 'test', url: 'test' } }
      expect(response).to have_http_status(:found)
    end
    it 'createアクションに不適切なパラメータを送るとレンダリングされる' do
      post rallies_path, headers: @headers, params: { rally: { title: '' } }
      expect(response.status).to eq 422
  ...
  describe 'DELETE #destroy' do
    it 'destroyアクションにリクエストすると正常にレスポンスが返ってくる' do
      delete rally_path(@rally), headers: @headers
      expect(response).to have_http_status(:found)
    end
  end
  describe 'GET #rally_lists' do
    it 'rally_listsアクションにリクエストすると正常にレスポンスが返ってくる' do
      get rally_lists_rallies_path(@user), headers: @headers
      expect(response.status).to eq 200
    end
  ...
  describe 'GET #ranking' do
    it 'rankingアクションにリクエストすると正常にレスポンスが返ってくる' do
      get ranking_rallies_path(@user), headers: @headers
      expect(response.status).to eq 200
    end
  end
end
